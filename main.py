...
class MyApp(QMainWindow):
    ...
    def make_manual_login(self):
        QMessageBox.information(self, '手動でログイン', "「ブラウザ起動」クリック後メルカリにログインしてくださいログイン後ブラウザを閉じないでください。")
    ...
    def decrease_prices(self):
        ...
        if interval_from > 0 and interval_to > 0 and interval_to > interval_from:
            ...
            self.msg_label.setText(f"商品を更新しています 1 / {self.marked_items}")
    ...
    def update_loader(self, updt):
        ...
        if is_successful:
            ...
            self.error_label.setText(f"更新出来ませんでした {i+1}")
    ...
    def get_listings(self):
        if self.driver:
            ...
            self.update_dialog3.setWindowTitle("商品を読み込んでます")
            self.update_dialog3.resize(350, 150)
            self.msg_label3 = QLabel("出品中の商品・非公開商品・下書き商品を取り込んでいます\nこれには時間がかかります。\nしばらくお待ち下さい・・・")
            self.ok_btn3 = QPushButton("閉じる")
            ...
            self.thread.login_signal.connect(lambda: QMessageBox.warning(self, '再度お試し下さい', 'ログインしているか確認してから、もう一度試してください。'))
    ...
    def update_listings(self, n_listings):
        if len(n_listings) > 0:
            self.msg_label3.setText("全ての商品の取り込みが完了しました!")
        else:
            self.msg_label3.setText("商品を読み込み出来ません、再度お試しください")
    ...
    def show_listings(self):
        ...
        for index, listing in enumerate(self.all_listings):
            ...
            product_table_item.setToolTip("商品IDクリックで商品ページが開きます")
                ...
                try:
                    ...
                    if self.listings[r]['source'] == 'listings':
                        if self.listings[r]['action'] == 2:
                            ...
                            self.listings[r]['status'] = "✓公開→公開停止中"
                        ...
                        self.listings[r]['status'] = "✓公開停止中→公開"
